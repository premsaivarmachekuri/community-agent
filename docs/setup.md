...
| `SEARCH_MODEL`             | No        | OpenAI model for web search (default: `gpt-4o-mini`). Web search uses OpenAI's native `webSearch` tool via the AI Gateway              |
| `SEARCH_DOMAINS`           | No        | Comma-separated domains to scope web search (e.g. `docs.example.com,partner.com`). Passed to the search model via prompt               |
